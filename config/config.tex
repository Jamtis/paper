...
\makeatletter
\newcommand{\defineenvironment}[2][\@nil]{%
    \expandafter\let\csname#2\endcsname\relax
    \makeatletter
    \expandafter\let\csname c@#2\endcsname\relax
    \declaretheorem[name=\makefirstuc{#2},style=boldstyle]{#2}%
    \ifx#1\@nil
        \crefname{#2}{#2}{#2{}s}
    \else
        \crefname{#2}{#2}{#1}
...
\defineenvironment{construction}
\defineenvironment{notation}
\defineenvironment{assumption}
\defineenvironment{question}
\defineenvironment{remark}
\defineenvironment{observation}
\defineenvironment{lemma}
\defineenvironment{theorem}
\defineenvironment[corollaries]{corollary}
\defineenvironment{claim}
\defineenvironment{fact}
...
% environment for hybrid proofs
\newif\ifitemstar
    \def\customlabelstring{}
    \newcommand{\hybridcustomref}{\arabic{enumi}}
    \newcommand{\hybridcustomlabel}{%
    \textbf{Game{\ifitemstar{*}\fi} \(\hybridcustomref\)\customlabelstring:}%
}
\newcommand{\hybriditem}{}
\newcommand{\hybriditemstar}{}
% the following enivorment takes an optional argument specifying the a template as which an item gets referenced, i.e., \ref{item:one} = "this is the \arabic{enumi} (first) item"
% the \hybriditem(star) command takes an optional argument specifying the name of the hybrid; displayed in parenthesis
\let\hybrids\relax
\newenvironment{hybrids}[1][]{%
    \begingroup%
    \ifthenelse{\equal{#1}{}}{}{%
        \renewcommand{\hybridcustomref}{#1}%
    }
    \renewcommand{\hybriditem}[1][]{%
        \ifthenelse{\equal{##1}{}}{}{%
            \def\customlabelstring{ (##1)}
        }
        \item%
        \def\customlabelstring{}%
    }%
    \renewcommand{\hybriditemstar}[1][]{%
        \global\itemstartrue%
        \hybriditem[##1]%
        \global\itemstarfalse%
    }%
    \begin{enumerate}[start=0,wide=0pt,label={\protect\hybridcustomlabel},ref={\hybridcustomref}]% \protect seems important
        %\begin{enumerate}[start=0,wide=0pt,ref={\hybridcustomref}]% \protect seems important
        }{%
    \end{enumerate}%
    \endgroup%
}
\newcommand\currenthybrid\hybridcustomref
\newcommand\previoushybrid{\the\numexpr\value{enumi}-1\relax}
...
% math environment that breaks across pages
...
%% tikz
% \usetikzlibrary{patterns} % incompatible with tikzpeople -> jester -> pattern
\usetikzlibrary{calc,shapes.misc,shapes.callouts,tikzmark,decorations.pathreplacing,calligraphy,graphs,arrows.meta,positioning}
\tikzset{
...
%% auto cref / text upright in display mode
...
%% ifthen lncs for math mode
\setcommand{\ifthenlncs}[2]{%
    \ifthenelse{\boolean{lncs}}{#1}{#2}%
}

%% keep the title capitalization as it is
