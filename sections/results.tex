...
\begin{proof}
    ...
    \begin{sitemize}
        ...
        \item \(\Enc\parr{\pk,m,t}\):
        \begin{sitemize}
            \item sample \(r_1,r_2,r_\VUF \gets \bit^\secpar\),
            \item generate \(\parr{\pk_\VUF,\sk_\VUF} \coloneqq \Gen_\VUF\parr{1^{\secpar};r_\VUF}\),
            \item set \(c_1 \coloneqq \Enc'\parr{\pk'_1,m;r_1} \in \bit^{2\secpar}\),
            \item set \(c_2 \coloneqq \Enc'\parr{\pk'_2,m||r_\VUF;r_2} \in \bit^{2\secpar}\),
            \item set \(\parr{\sigma_\VUF,\pi_\VUF} \coloneqq \Eval_\VUF\parr{\sk_\VUF,t||c_1||c_2}\),
            \item prove \(\pi \gets \Prove\parr{s_{\pk,c_1,c_2,\pk_\VUF},w}\) with witness \(w \coloneqq \parr{m,r_1,r_2,r_\VUF}\) where \(s_{\pk,c_1,c_2,\pk_\VUF}\) is following statement
            \begin{bralign}\label{eq:consistency2}
                &\ \parr{\begin{array}{c}
                    \exists \underline{m,r_1,r_2,r_\VUF} :
                    c_1 = \Enc\parr{\pk'_1,\underline{m;r_1}}
                    \wedge
                    c_2 = \Enc\parr{\pk'_2,\underline{m||r_\VUF;r_2}}
                    \\
                    \wedge\ 
                    \parr{\pk_\VUF,\cdot} = \Gen_\VUF\parr{1^\secpar;\underline{r_\VUF}}
                \end{array}}
                \\\label{eq:trapdoor2}
                \vee
                &\ \parr{
                    \exists \underline{R} :
                    C = \Commit\parr{\pp,c_1||c_2||\pk_\VUF||1;\underline{R}}
                }
                \ ,
            \end{bralign}
            \item output \(c \coloneqq \parr{t,c_1,c_2,\pk_\VUF,\pi,\sigma_\VUF,\pi_\VUF}\),
        \end{sitemize}
        Note that with an honestly generated public key \cref{eq:trapdoor2} cannot be fulfilled.

        \item \(\Dec\parr{\sk,c}\):
        \begin{sitemize}
            \item parse \(\parr{t,c_1,c_2,\pk_\VUF,\pi,\sigma_\VUF,\pi_\VUF} \coloneqq c\),
            \item if \(\Verify_\NIWI\parr{s_{\pk,c_1,c_2,\pk_\VUF},\pi} = 0\) output \(\bot\),
            \item if \(\Verify_\VUF\parr{\pk_\VUF,t||c_1||c_2,\sigma_\VUF,\pi_\VUF} = 0\) output \(\bot\),
            \item output \(\widetilde{m} \coloneqq \Dec'\parr{\sk'_1,c_1}\).
        \end{sitemize}
    ...
    \begin{hybrids}
        ...
        \marginnote{randomness \(r^*_\VUF\) is no longer used}
        ...
        \marginnote{How to deal with pcdiO? Obliviously sample \(\pk_\VUF\)?}
